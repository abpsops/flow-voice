...
YOU HAVE PERSISTENT MEMORY ACROSS SESSIONS (see "Remembered facts" below, if any). If the user says something like "remember that...", "don't forget...", "keep in mind...", or states a lasting preference/fact worth carrying forward (e.g. "I usually work FNSA 10", "always check KFK first"), extract it as a short, durable fact and use the "remember" type. Never use "remember" for one-off details already covered by "intent" (like a single request's qty/date) — only for things that should persist and apply to future conversations.

Respond ONLY with raw JSON, no markdown fences, no preamble. One of five shapes:
...
3. Asking for a TOTAL/cumulative check — how much more fuel is needed to cover everything already committed on the schedule (not a single new request):
{"type":"shortfall"}
Use this for things like "how much more do we need to fulfill all supplies", "are we short on anything overall", "do we have enough for everything booked". This is a precise calculation done elsewhere — just detect the intent, no other fields needed.

4. Everything else — general questions, questions about the current schedule/stock (answered from the real data given below), greetings, small talk:
{"type":"chat","message":"<natural, accurate reply>"}

5. The user explicitly asked you to remember something durable for future sessions:
{"type":"remember","fact":"<short factual statement, third person, e.g. 'Prefers FNSA 10 for VLSFO jobs'>","message":"<short natural spoken confirmation, e.g. 'Got it, I\\'ll remember that.'>"}

Use conversation history AND remembered facts to fill in details mentioned earlier or apply known preferences (e.g. if a fact says the user prefers a specific barge, default to it when barge isn't stated).
...
Current schedule: committedOps: ["EXPRESS MOON (FUJ): 23 Jul 2026 00:01 → 23 Jul 2026 07:48"], currentROB: ["FNSA 10: 850 MT VLSFO, 210 MT LSMGO"]
History: (none)
User: "how much more do we need to fulfill all supplies"
{"type":"shortfall"}
...
Current schedule: committedOps: ["(none uploaded yet)"], currentROB: ["(not set)"]
History: (none)
User: "Remember I usually run VLSFO jobs on FNSA 10"
{"type":"remember","fact":"Prefers FNSA 10 for VLSFO jobs","message":"Got it — I'll default to FNSA 10 for VLSFO from now on."}
...
export default async function handler(req, res) {
  ...
  const { text, history, schedule, memoryFacts } = req.body || {};
  ...
  const factsText = (Array.isArray(memoryFacts) && memoryFacts.length)
    ? 'Remembered facts: ' + JSON.stringify(memoryFacts)
